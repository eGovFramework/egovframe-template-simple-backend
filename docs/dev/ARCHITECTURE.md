...
### 맥락
기존 `ResultVO`의 `Map<String, Object>` 기반 응답이 타입 안정성이 부족함

### 결정
- 신규 API: `IntermediateResultVO<T>` 사용 (제네릭 기반)
- 기존 API: `ResultVO` 유지 (점진적 마이그레이션)
- 최종 목표: 모든 API를 `IntermediateResultVO<T>`로 통일
...
### 결정
- Request DTO / Response DTO를 별도 패키지에 분리
- `dto/request/`: 요청 바인딩용
- `dto/response/`: 응답 전용 (`@Builder`, `from()` 팩토리 메서드)
- 도메인 모델(Board, BoardVO)은 내부 레이어에서만 사용
